        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl rounded-[2.5rem] border border-slate-100">
                <div class="p-10 md:p-14 text-gray-900">
                    <form method="POST" action="{{ route('admin.packages.store') }}" class="space-y-10">
                        @csrf

                        <!-- Nama Paket -->
                        <div>
                            <x-input-label for="name" :value="__('Nama Paket')" class="text-[10px] font-black uppercase tracking-[0.2em] mb-4 text-slate-400" />
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full py-4 text-lg font-bold border-slate-100 italic" :value="old('name')" required autofocus placeholder="Contoh: Paket Exclusive Gold" />
                        ...
                        <div>
                            ...
                            <x-text-input id="price" name="price" type="number" class="mt-1 block w-full py-4 text-lg font-bold border-slate-100 italic" :value="old('price')" required placeholder="99000" />
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" class="mt-1 block w-full border-slate-100 focus:border-indigo-500 focus:ring-indigo-500 rounded-2xl shadow-sm text-sm p-4 h-32 italic" placeholder="Deskripsi paket untuk menarik minat user...">{{ old('description') }}</textarea>
                            ...
                            <div class="space-y-4">
                                <template x-for="(feature, index) in features" :key="index">
                                    <div class="flex gap-4 items-center">
                                        <div class="flex-grow">
                                            <input type="text" name="features[]" x-model="features[index]" class="w-full border-slate-100 focus:border-indigo-500 focus:ring-indigo-500 rounded-2xl shadow-sm text-sm p-4 italic" placeholder="Fitur paket..." required>
                                        </div>
