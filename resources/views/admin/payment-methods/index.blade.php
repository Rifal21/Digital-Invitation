        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            <div class="flex justify-between items-end mb-10">
                <div>
                     ...
                     <h4 class="text-4xl font-serif font-black text-[#0F172A]">Metode Pembayaran</h4>
                </div>
                <button @click="window.location.href='{{ route('admin.payment-methods.create') }}'" class="px-8 py-4 bg-[#0F172A] text-[#C5A267] rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-2xl hover:bg-slate-900 transition duration-500 active:scale-95">
                    Tambah Provider
                </button>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach($methods as $method)
                    <div class="bg-white rounded-[2.5rem] p-10 shadow-[0_30px_60px_rgba(0,0,0,0.02)] border border-slate-50 relative group transition hover:-translate-y-2">
                        ...
                        <div class="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center text-[#0F172A] mb-8 border border-slate-100 group-hover:rotate-12 transition shadow-sm">
                             @if($method->qr_image)
                        ...
                        <h3 class="text-2xl font-serif font-black text-[#0F172A] mb-4 italic">{{ $method->name }}</h3>
                        
                        <div class="space-y-4 mb-10">
                            <div>
                                <p class="text-[8px] font-black text-slate-300 uppercase tracking-widest mb-1">Account Number</p>
                                <p class="text-lg font-black text-[#0F172A] tracking-tighter">{{ $method->account_number }}</p>
                            </div>
                            <div>
                        ...
                        <div class="flex items-center gap-4 pt-8 border-t border-slate-50">
                            <a href="{{ route('admin.payment-methods.edit', $method) }}" class="text-[9px] font-black uppercase tracking-widest text-[#0F172A]/40 hover:text-[#0F172A] transition">Sunting</a>
