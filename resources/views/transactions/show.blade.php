            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-10 items-start">
                ...
                <div class="lg:col-span-2 space-y-6" data-aos="fade-right">
                    <div class="bg-white rounded-[2.5rem] shadow-[0_40px_80px_rgba(0,0,0,0.02)] border border-slate-100 overflow-hidden">
                        ...
                        <div class="p-8 md:p-12 bg-[#FDF8F0] border-b border-slate-50 flex flex-col md:flex-row justify-between items-center gap-6">
                            <div>
                                ...
                                <h2 class="font-serif text-2xl md:text-3xl font-black italic text-[#0F172A] tracking-tighter">{{ $transaction->invoice_number }}</h2>
                            </div>
                        ...
                        <div class="p-8 md:p-12">
                             <div class="flex flex-col md:flex-row justify-between items-center gap-8 mb-12">
                                 <div>
                                     ...
                                     <h3 class="text-xl md:text-2xl font-black text-[#0F172A] uppercase tracking-tight">{{ $transaction->package->name }}</h3>
                                 </div>
                                 <div class="text-center md:text-right">
                                      <p class="text-[8px] font-black uppercase tracking-[0.3em] text-slate-400 mb-1">Total Investment</p>
                                      <h3 class="font-serif text-3xl font-black text-[#0F172A] tracking-tighter italic">IDR {{ number_format($transaction->total_amount, 0, ',', '.') }}</h3>
                                 </div>
                             </div>

                             <div class="space-y-6 pt-10 border-t border-slate-50">
                                  <h4 class="text-[9px] font-black text-[#0F172A] uppercase tracking-[0.3em] mb-4">Rincian Paket:</h4>
                    ...
                    <div class="p-8 rounded-[2.5rem] bg-slate-50 border border-slate-100 flex flex-col md:flex-row items-center gap-8 opacity-70">
                         <div class="w-14 h-14 rounded-2xl bg-white flex items-center justify-center text-[#0F172A] text-lg shadow-sm flex-shrink-0">
                ...
                <div class="lg:col-span-1 space-y-6 sticky top-28" data-aos="fade-left">
                    <div class="bg-white rounded-[2.5rem] shadow-[0_40px_80px_rgba(0,0,0,0.02)] border border-slate-100 p-8 md:p-10">
                        ...
                        <div class="mb-10 text-center">
                            <h4 class="text-[10px] font-black text-[#0F172A] uppercase tracking-[0.3em] mb-8">Tujuan Transfer:</h4>
                            
                            @if($method)
                                <div class="bg-[#FDF8F0] p-6 rounded-3xl border border-[#C5A267]/30 group">
                                    <div class="flex items-center justify-center gap-3 mb-6">
                                         <div class="w-8 h-8 rounded-xl bg-[#0F172A] text-[#C5A267] flex items-center justify-center text-[10px] shadow-lg group-hover:scale-110 transition">
                                             <i class="fas {{ $method->qr_image ? 'fa-qrcode' : 'fa-university' }}"></i>
                                         </div>
                                         <span class="text-[11px] font-black uppercase text-[#0F172A] tracking-widest">{{ $method->name }}</span>
                                    </div>
                                    <div class="space-y-4">
                                         <div class="bg-white p-4 rounded-2xl border border-slate-100">
                                              ...
                                              <div class="flex items-center justify-between">
                                                  <span class="text-base font-black text-[#0F172A] tracking-tighter">{{ $method->account_number }}</span>
                                         ...
                                         <div>
                                              ...
                                              <p class="text-[11px] font-black text-[#0F172A] uppercase tracking-tighter italic">{{ $method->account_name }}</p>
                                         </div>
                                         @if($method->qr_image)
                                            <div class="pt-2">
                        ...
                        <div class="pt-8 border-t border-slate-50">
                            <h4 class="text-[10px] font-black text-[#0F172A] uppercase tracking-[0.3em] mb-6">Unggah Bukti:</h4>
                            
                            <form action="{{ route('transactions.upload', $transaction) }}" method="POST" enctype="multipart/form-data" @submit="uploading = true">
                                @csrf
                                <div class="mb-6">
                                    <label for="p_proof" class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-100 rounded-3xl hover:border-[#0F172A]/20 hover:bg-slate-50 transition cursor-pointer group">
                                ...
                                <button type="submit" :disabled="uploading" class="w-full py-5 rounded-2xl bg-[#0F172A] text-[#C5A267] font-black text-[11px] uppercase tracking-[0.4em] shadow-xl hover:bg-slate-900 transition-all duration-500 active:scale-95 disabled:opacity-50">
