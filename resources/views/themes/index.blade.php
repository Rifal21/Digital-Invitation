...
<body class="antialiased overflow-x-hidden">
    ...
    <nav class="nav-glass sticky top-0 z-[100] border-b border-slate-100/50 flex justify-between items-center px-6 md:px-20 h-20 md:h-24 shadow-sm">
        <a href="/" class="flex items-center gap-4 group">
            ...
            <div>
                <span class="text-xl md:text-2xl font-serif font-black tracking-widest uppercase leading-none block">Memora</span>
        ...
        <div class="flex items-center gap-3">
            ...
            @else
                ...
                <a href="{{ route('register') }}" class="hidden sm:block text-[10px] font-black uppercase tracking-widest px-8 py-3 bg-[#4D243D] text-[#EDD4B2] rounded-2xl shadow-xl hover:bg-slate-900 hover:scale-105 active:scale-95 transition-all">Bergabung</a>
            @endauth
        </div>
    </nav>
    ...
    <header class="relative pt-32 pb-20 md:pt-48 md:pb-32 text-center overflow-hidden">
        ...
        <div class="absolute -top-40 -right-40 w-96 h-96 bg-[#EDD4B2]/10 rounded-full blur-[100px]"></div>
        ...
        <div class="max-w-5xl mx-auto px-8 relative z-10" data-aos="fade-up">
            ...
            <h1 class="font-serif text-5xl md:text-8xl font-black mb-10 leading-[1.1] tracking-tight text-[#4D243D]">
                Lahir dari <span class="italic text-slate-300">Seni</span>, <br>
                Dirancang untuk <span class="text-[#EDD4B2] italic">Cinta.</span>
            </h1>
            <p class="max-w-2xl mx-auto text-slate-500 text-lg md:text-xl font-medium italic leading-relaxed opacity-80">
                Pilih mahakarya desain kami yang akan menjadi saksi bisu keindahan janji suci dan perjalanan cinta abadi Anda oleh <span class="text-[#4D243D] font-bold">FKStudio</span>.
            </p>
                ...
                <div class="group relative" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                    ...
                    <div class="theme-card-frame mb-10 relative group">
                            ...
                            @else
                                <div class="w-full h-full p-12 flex flex-col justify-center items-center text-center space-y-8 bg-gradient-to-br from-[#FDFBFA] to-[#F5EFE9]">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[8px] uppercase tracking-[0.5em] font-black opacity-30">Artisan Series</p>
                                    </div>
                        ...
                        <div class="glass-overlay absolute inset-0 opacity-0 group-hover:opacity-100 transition-all duration-700 flex flex-col justify-center items-center p-12 translate-y-full group-hover:translate-y-0 shadow-2xl">
                            <i class="fas fa-eye text-[#EDD4B2] text-4xl mb-10 transform -translate-y-10 group-hover:translate-y-0 transition-transform duration-1000 opacity-50"></i>
                            
                            <div class="space-y-4 w-full px-4 text-center">
                                <a href="{{ route('themes.preview', $theme->slug) }}" target="_blank" class="block w-full py-5 bg-[#EDD4B2] text-[#4D243D] text-[10px] font-black uppercase tracking-[0.4em] rounded-[2rem] shadow-2xl hover:bg-white hover:scale-105 transition-all duration-500">
                                    <i class="fas fa-play text-[8px] mr-2 opacity-50"></i> Live Preview
                                </a>
                                @auth
                                    @if(Auth::user()->invitations()->where('theme', $theme->slug)->exists())
                                        <div class="py-4 text-[#EDD4B2] text-[9px] font-black uppercase tracking-widest text-center opacity-70">
                                    ...
                                    @else
                                        <form action="{{ route('invitations.store') }}" method="POST">
                                            ...
                                            <button type="submit" class="w-full py-5 bg-white/10 border-2 border-white/20 text-white text-[10px] font-black uppercase tracking-[0.4em] rounded-[2rem] hover:bg-white hover:text-[#4D243D] hover:border-white transition-all duration-500">
                                                Gunakan Desain 
                                            </button>
                                        </form>
                                    @endif
                                @else
                                    <a href="{{ route('register', ['theme' => $theme->slug]) }}" class="block w-full py-5 bg-white/10 border-2 border-white/20 text-white text-[10px] font-black uppercase tracking-[0.4em] rounded-[2rem] hover:bg-white hover:text-[#4D243D] hover:border-white transition-all duration-500">
                                        Pesan Tema Ini
                                    </a>
                                @endauth
                            </div>
